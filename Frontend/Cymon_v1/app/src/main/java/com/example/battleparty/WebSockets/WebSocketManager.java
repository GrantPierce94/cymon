...
import com.example.battleparty.Player;
...
public class WebSocketManager {
    ...
    //new approach for websocket threading: one client for each ServerEndpoint
    //in the backend: FriendController, ItemController, and PartyController
    private MyWebSocketClient itemControllerClient;
    private MyWebSocketClient partyControllerClient;
    private MyWebSocketClient friendControllerClient;
     ...
     */
    public void connectWebSocket(String serverUrl, UtilEnum.CONTROLLER key) {
        WebSocketClient temp = null;
        switch (key) {
            case FRIENDCONTROLLER:
                temp = friendControllerClient;
                if (temp == null || !temp.isOpen()) {
                    try {
                        URI serverUri = URI.create(serverUrl);
                        friendControllerClient = new MyWebSocketClient(serverUri);
                        friendControllerClient.connect();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
                break;
            case ITEMCONTROLLER:
                temp = itemControllerClient;
                if (temp == null || !temp.isOpen()) {
                    try {
                        URI serverUri = URI.create(serverUrl);
                        itemControllerClient = new MyWebSocketClient(serverUri);
                        itemControllerClient.connect();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            case PARTYCONTROLLER:
                temp = partyControllerClient;
                if (temp == null || !temp.isOpen()) {
                    try {
                        URI serverUri = URI.create(serverUrl);
                        partyControllerClient = new MyWebSocketClient(serverUri);
                        partyControllerClient.connect();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
                break;
            default:
                Log.d("[connectWebSocket] ", "enter a valid endpoint enum");
        }
        Log.d("[connectWebSocket] ", key.name());
    }
     ...
     */
    public void sendMessage(String message, UtilEnum.CONTROLLER key) {
        WebSocketClient temp = null;
        ...
        switch (key) {
            ...
            case PARTYCONTROLLER:
                ...
                break;
            default:
                Log.d("[sendMessage] ", "enter a valid endpoint enum");
        }
     ...
     * WebSocket server.
     */
    public void disconnectWebSocket(UtilEnum.CONTROLLER key) {
        WebSocketClient temp = null;
        switch (key) {
            case FRIENDCONTROLLER:
                temp = friendControllerClient;
            ...
            case PARTYCONTROLLER:
                ...
                break;
            default:
                Log.d("[disconnectWebSocket] ", "enter a valid endpoint string");
        }
        if (temp != null) {
            temp.close();
        }
