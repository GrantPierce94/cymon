...
public class monController {
    ...
    @GetMapping("/player")
    public @ResponseBody HashMap<String,Player> getAllPlayers() {return playerList;}
    ...
    @PostMapping("/player")
    public @ResponseBody String createPlayer(@RequestBody Player player) {
        System.out.println(player);
        System.out.println(player.getMonIn());
        Mon mon = monList.get(player.getMonIn());

        if(!mon.isClaimed()) {
            mon.setClaimed();
            player.setMonster(mon);
        }
        else{
            player.setMonster(null);
        }
        playerList.put(player.getName(), player);
        return "New Player "+ player.getName() + " Saved";
    }
